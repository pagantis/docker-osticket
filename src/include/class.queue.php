...
class CustomQueue extends VerySimpleModel {
  ...
  /**
    ...
    /**
     * Retrieve a QuerySet instance based on the type of object (docker) of
     ...
     *      visible queues.
     * $pid - <int> parent_id of docker queue. Default is zero (top-level)
     */
    ...
    static function getConditionsFromPost(array $vars, $myid, $root='Ticket') {
        ...
        foreach (@$vars['conditions'] as $i=>$id) {
            ...
            if (!isset($fields[$name]))
                // No such field exists for this queue docker type
                continue;
...
extends VerySimpleModel {
    ...
    function update($vars, &$errors=array()) {
        ...
        $this->name = $vars['name'];
        if (isset($vars['docker']))
            $this->root = $vars['docker'];
        elseif (!isset($this->root))
            $this->root = 'T';

        $fields = CustomQueue::getSearchableFields($this->getRoot($vars['docker']));
        $columns = array();
