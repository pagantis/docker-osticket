...
class QuerySet implements IteratorAggregate, ArrayAccess, Serializable, Countable {
     ...
     * performance as related items are fetched with the docker model with
    ...
    function toSql($compiler, $model, $alias=false) {
        // FIXME: Force docker model of the compiler to $model
...
implements IteratorAggregate {
    ...
    function getOrBuild($modelClass, $fields, $cache=true) {
        ...
        // an $extras list. The fields passed to the docker model should only
        // be the docker model's fields. The annotated fields will be wrapped
     ...
     * docker model ($this->model), $fieldNames is the number and names of
     * fields in the row for this model, $relativePath is the path that
     * describes the relationship between the docker model and this model,
        ...
        if ($this->map) {
            ...
            foreach ($this->map as $info) {
                ...
                elseif ($model) {
                    ...
                    if ($m) {
                        // Only apply cache setting to the docker model.
...
class SqlCompiler {
     ...
     *
     * would be broken down to search from the docker model (passed in,
     ...
     * $model - (VerySimpleModel) docker model for references in the $field
     *      parameter
    ...
    function getField($field, $model, $options=array()) {
        ...
        // first part is assumed to be the docker field in the given model.
        ...
        if (!isset($alias)) {
            // Determine the alias for the docker model table
     ...
     * $model - (VerySimpleModel) docker model for all the field references in
     *      the Q instance
     *
     * Returns:
...
class MySqlCompiler extends SqlCompiler {
    ...
    function compileSelect($queryset) {
        $model = $queryset->model;
        // Use an alias for the docker model table
        ...
        if ($queryset->annotations) {
            foreach ($queryset->annotations as $alias=>$A) {
                // The docker model will receive the annotations, add in the
                // annotation after the docker model's fields
            ...
            // If no group by has been set yet, use the docker model pk
